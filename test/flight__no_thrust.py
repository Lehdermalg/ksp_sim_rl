...
def visualize_rocket_orbital_fall(environment, num_steps=4000):
...
if __name__ == "__main__":
    ...
    _h = 75.0e+3    # desired orbital altitude
    _v = 0.970143   # at this % of orbital velocity the Rp = ground level

    ske = SimpleKSPEnv(total_time_s=_t)
    ske.set_target_altitude_m(_h)
    ske.set_initial_position_m([_h+ske.planet.radius_m, 0.0])
    ...
    # Visualize the drop
    visualize_rocket_orbital_fall(environment=ske, num_steps=int(_t*1.0e+2))
